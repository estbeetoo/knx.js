...
const KnxDatagram = require('./KnxDatagram');
const KnxReceiver = require('./KnxReceiver');
...
KnxReceiverTunneling.prototype.ProcessConnectResponse = function (/*buffer*/ datagram) {
  ...
  if (knxDatagram.channel_id == 0x00 && knxDatagram.status == 0x24) {
    console.error('KnxReceiverTunneling: Received connect response - No more connections available');
    const connectAgain = () => this.connection.Connect();
    timeout((resolve) => this.connection.Disconnect(resolve), CONNECT_TIMEOUT).then(connectAgain).catch(connectAgain);
  } else {
