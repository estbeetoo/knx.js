 ...
 */
const debug = require('debug')('knx.js:KnxReceiver');
...
KnxReceiver.prototype.ProcessCEMI = function (/*KnxDatagram*/ datagram, /*buffer*/ cemi) {
  try {
    ...
    debug([
      cemi.toString('hex'),
      `Event Header Length: ${datagram.header_length}`,
      `Event Protocol Version: ${datagram.protocol_version}`,
      `Event Service Type: 0x${datagram.service_type.toString('hex')}`,
      `Event Total Length: ${datagram.total_length}`,
      `Event Message Code: ${datagram.message_code}`,
      `Event Aditional Info Length: ${datagram.additional_info_length}`,
      datagram.additional_info_length > 0 && `Event Additional Info: 0x${datagram.additional_info.toString('hex')}`,
      `Event Control Field 1: ${datagram.control_field_1}`,
      `Event Control Field 2: ${datagram.control_field_2}`,
      `Event Source Address: ${datagram.source_address}`,
      `Event Destination Address: ${datagram.destination_address}`,
      `Event Data Length: ${datagram.data_length}`,
      `Event APDU: 0x${datagram.apdu.toString('hex')}`,
      `Event Data: ${datagram.data.toString('hex')}`,
    ].filter(Boolean).join('\n'));
    ...
    switch (type) {
      ...
      case 4:
        ...
        break;
      default:
        debug('Unknown type[' + type + '] received in datagram[' + datagram.data.toString('hex') + ']');
        break;
    }
  } catch (e) {
